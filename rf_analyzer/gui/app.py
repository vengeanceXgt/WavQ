...
if uploaded is not None:
    ext = os.path.splitext(uploaded.name)[1] or ".iq"
    ...
    if st.button("Run Analysis"):
        ...
        try:
            ...
            if result["synchronization"].get("timing_status") == "locked":
                ...
                sps = int(result["synchronization"].get("symbol_rate_sps") or 4)
                ...
                with p4:
                    st.plotly_chart(plot_cfo_trajectory(cfo), use_container_width=True)

                # We don't have frame correlation strictly from the JSON but we can generate an unavailable plot
                st.plotly_chart(plot_frame_correlation(None), use_container_width=True)
            else:
                p3, p4 = st.columns(2)
                with p3:
                    st.plotly_chart(plot_constellation(None), use_container_width=True)
                with p4:
                    st.plotly_chart(plot_cfo_trajectory(result["synchronization"].get("cfo_hz")), use_container_width=True)
